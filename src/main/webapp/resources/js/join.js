/* 공백 체크 정규식 */
	var emp = /\s/g;
	/* 아이디 공백 체크 정규식 */
	var idEmp = /^[A-Za-z0-9_\-]{5,20}$/; // 영문 대 소문자, 숫자 _-만 입력 가능 (5에서 20자까지)
	/* 비밀번호 공백 체크 정규식 */
	...
	var address2 = $('#userAddress2'); 
		...
		$('#idCheckBtn').on('click',function(){
			if($('#userid').val()==''){
				...
				$('#id_check').text('5~20자의 영문, 숫자만 가능합니다.').css('color','red');
			}else if($('#userid').val()!=''){
					...
					success : function(data){
						...
						if(data=='1'){
							...
							if(idEmp.test(userid)){
								...
								$('#id_check').text('아이디는 숫자 또는 영소문자 5~20자리만 가능합니다. ').css('color','red');
								$('#usercheck').attr("disabled", true);
							}
							id_Check = 1;
						}
		...
		$('#userpw').blur(function(){
			if(pwEmp.test($('#userpw').val())){
				...
				$('#pw_check').text('4~12자의 숫자, 영문자로만 사용 가능합니다.').css('color', 'red');
